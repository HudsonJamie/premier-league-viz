...
p_points <- ggplot(tidy_weekly_df_points, aes(x = date, y = points, group = Team)) + 
  ...
  theme_minimal() +
  scale_x_date(breaks = seq(min(tidy_weekly_df_points$date),max(tidy_weekly_df_points$date), by="1 week"), date_labels = "%d-%m-%y") +
